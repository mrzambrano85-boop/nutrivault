...
export default function Tickets() {
  ...
  function handleFile(file: File) {
    if (!file.type.startsWith("image/")) {
      setErrorMsg(t("tick.err_not_image"));
      setEstado("error");
      return;
    }
  ...
  async function escanear() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t("tick.error_title"));
      setIngredientes(data.ingredientes ?? []);
      setEstado("resultado");
    } catch (err: unknown) {
      setErrorMsg(err instanceof Error ? err.message : t("tick.error_title"));
      setEstado("error");
    }
  }
  ...
  async function guardarEnDespensa() {
    ...
    try {
      const rows = ingredientes.map((ing) => ({
        ...
        categoria: ing.categoria, usuario_id: user.id,
      }));
  ...
  return (
    <Layout>
      <div className="space-y-6 max-w-2xl mx-auto">
        <header>
          <h1 className="text-3xl font-bold text-foreground">{t("tick.title")}</h1>
          <p className="text-muted-foreground mt-1">{t("tick.subtitle")}</p>
        </header>
        ...
        {estado === "idle" && (
          <Card className="border-dashed border-2 border-primary/30 hover:border-primary/60 transition-colors">
            <CardContent className="flex flex-col items-center justify-center p-16 text-center gap-6">
              ...
              <div>
                <h3 className="text-lg font-semibold">{t("tick.upload_title")}</h3>
                <p className="text-muted-foreground mt-1 text-sm">{t("tick.upload_desc")}</p>
              </div>
              <div className="flex gap-3 flex-wrap justify-center">
                <Button onClick={() => fileRef.current?.click()} data-testid="button-upload-image">
                  <Upload className="h-4 w-4 mr-2" /> {t("tick.btn_upload")}
                </Button>
                <Button variant="outline" onClick={() => cameraRef.current?.click()} data-testid="button-open-camera">
                  <Camera className="h-4 w-4 mr-2" /> {t("tick.btn_camera")}
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-4">
            <Card>
              <CardContent className="p-4">
                <img src={imagePreview} alt={t("tick.title")} className="w-full max-h-96 object-contain rounded-md bg-muted" data-testid="img-ticket-preview" />
              </CardContent>
            </Card>
            <div className="flex gap-3">
              <Button onClick={escanear} disabled={estado === "scanning"} className="flex-1" data-testid="button-scan-ticket">
                <ScanLine className="h-4 w-4 mr-2" />
                {estado === "scanning" ? t("tick.btn_scanning") : t("tick.btn_scan")}
              </Button>
              <Button variant="outline" onClick={reiniciar} disabled={estado === "scanning"}>
                <RotateCcw className="h-4 w-4 mr-2" /> {t("tick.btn_change")}
              </Button>
            </div>
            {estado === "scanning" && (
              <Card className="border-primary/20 bg-primary/5">
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <p className="text-sm text-primary font-medium">{t("tick.scanning_msg")}</p>
                </CardContent>
              </Card>
            )}
          </div>
            ...
            <Card>
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="font-semibold text-lg">{t("tick.found", { n })}</h2>
                  <Button variant="ghost" size="sm" onClick={reiniciar}>
                    <RotateCcw className="h-4 w-4 mr-1" /> {t("tick.btn_new")}
                  </Button>
                </div>
                {n === 0 ? (
                  <p className="text-muted-foreground text-sm">{t("tick.no_products")}</p>
                ) : (
                    ...
                    {ingredientes.map((ing, i) => (
                      <div key={i} className="py-3 flex items-center justify-between" data-testid={`row-ingredient-${i}`}>
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{ing.cantidad} {ing.unidad}</p>
                        </div>
                ...
                {n > 0 && (
                  <Button onClick={guardarEnDespensa} disabled={estado === "guardando"} className="w-full" data-testid="button-save-to-despensa">
                    <Check className="h-4 w-4 mr-2" />
                    {estado === "guardando" ? t("tick.btn_saving") : t("tick.btn_save", { n })}
                  </Button>
                )}
              </CardContent>
            </Card>
        ...
        {estado === "guardado" && (
          <Card className="border-primary/30 bg-primary/5">
            <CardContent className="flex flex-col items-center justify-center p-12 text-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-primary">{t("tick.success_title")}</h3>
                <p className="text-muted-foreground mt-1 text-sm">{t("tick.success_msg", { n })}</p>
              </div>
        ...
        {estado === "error" && (
          <Card className="border-destructive/30">
            <CardContent className="flex flex-col items-center justify-center p-10 text-center gap-4">
              ...
              <div>
                <h3 className="font-semibold text-destructive">{t("tick.error_title")}</h3>
                <p className="text-muted-foreground mt-1 text-sm">{errorMsg}</p>
              </div>
              <Button onClick={reiniciar} variant="outline">
                <RotateCcw className="h-4 w-4 mr-2" /> {t("tick.btn_retry")}
              </Button>
            </CardContent>
          </Card>
        )}
